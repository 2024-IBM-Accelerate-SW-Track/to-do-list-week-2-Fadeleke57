...
const Todos = ({ todos, deleteTodo }) => {
  const todoList = todos.length ? (
    todos.map((todo) => {
      return (
        <Grid key={todo.id}>
          <Card style={{marginTop:10, backgroundColor: `rgba(150, 22, 204, ${brightness[todo.urgency]})`}}>
            ...
            <ListItemButton component="a" href="#simple-list">
              <Checkbox 
                ...
                color="primary"
                />
              <ListItemText primary={`${todo.content} ${todo.location ? `( At ${todo.location})` : ""}`} secondary={todo.date}/>
            </ListItemButton>
          </Card>
        </Grid>
      );
